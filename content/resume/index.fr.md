---
title: "CV"
---

## Expérience

{{< Timeline >}}

{{< timelineItem icon="code" header="STEF" subheader="Ingénieur de Production SysOps" >}}
<img src="STEF-Logo.png" alt="STEF Logo" />
<strong>Site web : </strong><span><a href="https://www.stef.com/france/" target="_blank">STEF</a></span><br>
<strong>Localisation :</strong> Paris, France <br>
<strong>Date :</strong> 2024 - 2025
{{< /timelineItem >}}

{{< timelineItem icon="code" header="MetalMaker3D" subheader="Ingénieur Réseaux (stage)">}}
<img src="/resume/img/mm3d.jpeg" alt="MetalMaker3D Logo" />
<strong>Site web : </strong><span><a href="https://metalmaker3d.com/" target="_blank">MetalMaker3D</a></span><br>
<strong>Localisation :</strong> Cracovie, Poland <br>
<strong>Date :</strong> 2024
{{< /timelineItem >}}

{{< timelineItem icon="code" header="Videomusuem" subheader="System and Network administrator">}}
<img src="/resume/img/videomuseum-logo.jpg" width=248 height=248  alt="Videomuseum Logo" />
<strong>Site web : </strong><span><a href="https://www.videomuseum.fr/" target="_blank">Videomuseum</a></span><br>
<strong>Localisation :</strong> Paris, France <br>
<strong>Date :</strong> 2021 - 2024
{{< /timelineItem >}}

{{< timelineItem icon="code" header="Omega Agency" subheader="Développeur Full-Stack (stage)" >}}
<img src="/resume/img/Omega-Agency-logo.jpg" alt="Omega Agency Logo" />
<strong>Site web : </strong><span><a href="https://www.omegaagency.fr/" target="_blank">Omega Agency</a></span><br>
<strong>Localisation :</strong> Lille, France <br>
<strong>Date :</strong> 2021
{{< /timelineItem >}}

{{< /Timeline >}}

## Formation

{{< Timeline >}}

{{< timelineItem icon="graduation-cap" header="ESME Sudria" badge="Diplôme d'ingénieur">}}
<img src="/resume/img/esme-sudria-logo.png" width=248 height=248 alt="ESME Sudria Logo" />

<strong>Site web : </strong><span><a href="https://www.esme.fr/" target="_blank">esme.fr</a></span><br>
<strong>Date :</strong> 2021 - 2024
{{< /timelineItem >}}

{{< timelineItem header="Université Sorbonne Paris Nord" badge="DUT">}}
<img src="/resume/img/uspn-logo.png" alt="USPN Logo" />
<strong>Site web : </strong><span><a href="https://www.univ-spn.fr/" target="_blank">Université Sorbonne Paris Nord</a></span><br>
<strong>Date :</strong> 2019 - 2021
{{< /timelineItem >}}

{{< /Timeline >}}